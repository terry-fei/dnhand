...
module.exports.login = (user, callback) ->
  ...
  url = "http://nh.feit.me/rj?stuid=#{user.stuid}&pswd=#{user.pswd}"
...
module.exports.currentState = (user, callback) ->
  ...
  urllib.request stateUrl, options, (err, html, res) ->
    ...
    unless Array.isArray spans
      return callback 'can not get info'
    tmpState = {}
    for span in spans
      dom = cheerio(span)
...
module.exports.charge = (user, callback) ->
  ...
  _preRequest chargeUrl, headers, (err, preHtml) ->
    ...
    urllib.request checkNetCardStatusUrl, options, (err, html, res) ->
      ...
      ret = {errcode: -1, ka: user.cardNo, mi: user.cardSecret}

      if !!~ html.indexOf '生成时间'
        ret.errcode = 0
      else if !!~ html.indexOf '充值卡已被充值'
        ret.errcode = 1
        ret.errmsg = '充值卡已被充值'
      else if !!~ html.indexOf '充值卡不存在或已作废'
        ret.errcode = 4
        ret.errmsg = '充值卡不存在或已作废'
